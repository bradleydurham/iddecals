...
class VariantSelects extends HTMLElement {
  ...
  renderProductInfo() {
    ...
    fetch(`${this.dataset.url}?variant=${requestedVariantId}&section_id=${this.dataset.originalSection ? this.dataset.originalSection : this.dataset.section}`)
      ...
      .then((responseText) => {
        ...
        // BD code start
        const sku = document.getElementById(`bdsku-${this.dataset.section}`);
  ...
  // BD Code start
  updateSku(html) {
    const id = `bdsku-${this.dataset.section}`;
  ...
  setUnavailable() {
    ...
    // BD Code Start
    const bdsku = document.getElementById(`bdsku-${this.dataset.section}`);
    // BD code end

    if (!addButton) return;

    // BD code start
    if (bdsku) bdsku.classList.add('visibility-hidden');
    // BD code end
